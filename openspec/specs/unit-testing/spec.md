...
### Requirement: コアロジックのカバレッジ 100% 強制
システムは、`src/logic.ts` に対して 100% のテストカバレッジを機械的に強制しなければならない（MUST）。カバレッジの閾値は Vitest の設定ファイルに個別エントリとして定義し、1行でも未カバーのコードが存在する場合はテストスイートが失敗するものとする。

#### Scenario: カバレッジ閾値違反の検出
- **WHEN** `src/logic.ts` に未カバーのコード行が存在する状態で `npm test` を実行する
- **THEN** テストスイートがカバレッジ閾値違反のエラーメッセージとともに失敗する

#### Scenario: カバレッジ 100% 達成時のパス
- **WHEN** `src/logic.ts` の全コード行がテストでカバーされた状態で `npm test` を実行する
- **THEN** カバレッジチェックがパスし、テストスイートが正常終了する
