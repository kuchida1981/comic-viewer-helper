...
**Goals:**
- ソースコードブランチ（`master`および作業ブランチ）からビルド成果物を排除する。
- `master`ブランチへのプッシュ時に、GitHub Actionsで自動的にビルドとデプロイを行う。
...
### 3. .gitignore の更新
- **決定**: `master` ブランチにおいて `dist/` を `.gitignore` に追加し、既存のファイルを削除する。
...
- **[Risk] CIの失敗による配布停止** → ビルドが失敗すると `release` ブランチが更新されない。
  - **Mitigation**: `master` ブランチへのマージ前のCIチェック（既存の `build.yml`）を厳格に運用し、デプロイワークフロー自体の失敗も通知するようにする。
