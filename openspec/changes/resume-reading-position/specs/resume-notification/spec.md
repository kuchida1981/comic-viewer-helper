...
システムは、保存された読書位置がある場合、画面下部中央にトースト通知を表示しなければならない。
...
#### Scenario: 機能が無効の場合は表示しない
- **WHEN** resumeEnabled が false の状態でページが初期化された
- **THEN** システムはトースト通知を表示しない

### Requirement: トースト通知の配置を定義する
システムは、トースト通知を既存UIと干渉しない位置に配置しなければならない。
...
システムは、「続きから」ボタンをクリックすると保存された位置にジャンプしなければならない。
...
システムは、「最初から」ボタンをクリックすると1ページ目から読めるようにしなければならない。
...
システムは、一定時間経過後またはスクロール開始時に通知を自動削除しなければならない。
...
システムは、トースト通知を視認性が高く、操作しやすいスタイルで表示しなければならない。
