...
- [x] 5.1 `Navigator._stopAutoplay` の責務を整理し、最終ページ到達時の停止処理を改善
- [x] 5.2 `AutoplayControls` の入力バリデーション（上限99）を追加
- [x] 5.3 `AutoplayControls` の `keydown` イベント制御（`preventDefault` 等）を改善
- [x] 5.4 スペックおよび設計書の文言修正（タイポ修正、型表記の正確化、一貫性の確保）
- [x] 5.5 追加修正分の検証およびテスト実行
