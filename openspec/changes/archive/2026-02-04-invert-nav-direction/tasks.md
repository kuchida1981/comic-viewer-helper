...
- [x] 3.1 `<` ボタンで次のページへ、`>` ボタンで前のページへに動作が反転していることを確認する
- [x] 3.2 `<<` ボタンで最後のページへ、`>>` ボタンで最初のページへに動作が反転していることを確認する
- [x] 3.3 `ArrowLeft` キーで次のページへ、`ArrowRight` キーで前のページへに動作が反転していることを確認する
- [x] 3.4 `j`/`k`, `PageDown`/`PageUp`, `Space`/`Shift+Space` の動作は変わっていないことを確認する
- [x] 3.5 ホイールナビゲーションの動作は変わっていないことを確認する
- [x] 3.6 最終ページで `<` ボタンまたは `ArrowLeft` キーを操作した場合に Info モーダルが自動表示されることを確認する