...
- [x] 2.2 残りの UI ロジックを `src/main.js` に移行する
- [x] 2.3 `vite.config.mjs` を設定し、iife 形式でバンドルして `.user.js` を出力するようにする
- [x] 2.4 メタデータヘッダーを保持するためのプラグイン設定またはスクリプトを追加する
- [x] 2.5 `package.json` に `build` スクリプトを追加する
...
- [x] 4.2 GitHub 上でアクションが正常に動作し、テスト結果が報告されることを確認する
