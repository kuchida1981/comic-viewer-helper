...
**特に `src/logic.ts` については、カバレッジ 100% を維持しなければならず（MUST）、1行でも未カバーがある場合はテストを失敗させなければならない。**
...
#### Scenario: コアロジックのカバレッジ 100% 強制
- **WHEN** `npm test` を実行し、`src/logic.ts` に未カバーの行が存在する場合
- **THEN** Vitest はエラーを返し、プロセスは非ゼロの終了コードで終了する
