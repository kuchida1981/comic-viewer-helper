...
Store はアプリケーションのすべての重要な状態を保持し、変更が加えられた際には自動的に `localStorage` へ永続化し、登録されたリスナーへ通知しなければならない。
...
Store は初期化時に `localStorage` から保存された状態を読み込み、存在しない場合はデフォルト値を適用しなければならない。
