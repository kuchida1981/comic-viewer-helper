...
- [ ] 2.1 最終ページで `j` キーを押し、メタデータモーダルが自動で表示されることを確認
- [ ] 2.2 モーダルが既に開いている状態で最終ページの「次へ」を操作し、重複表示や異常な動作が起こらないことを確認
- [ ] 2.3 最初のページで `k` キーを押し、モーダルが表示されないことを確認（「前へ」方向は対象外）
- [ ] 2.4 最終ページ以外の途中ページで「次へ」を操作し、通常のページ遷移が正常に動作することを確認（回帰チェック）
